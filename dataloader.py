...
class YouTubeFace(Dataset):
    def __init__(self, path):
        data = scipy.io.loadmat(path)
        scaler = MinMaxScaler()
        # self.view1 = scaler.fit_transform(data['X'][0][0].astype(np.float32))
        # self.view2 = scaler.fit_transform(data['X'][1][0].astype(np.float32))
        # self.view3 = scaler.fit_transform(data['X'][2][0].astype(np.float32))
        # self.view4 = scaler.fit_transform(data['X'][3][0].astype(np.float32))
        # self.view5 = scaler.fit_transform(data['X'][4][0].astype(np.float32))
        self.view1 = data['X'][0][0].astype(np.float32)
        self.view2 = data['X'][1][0].astype(np.float32)
        self.view3 = data['X'][2][0].astype(np.float32)
        self.view4 = data['X'][3][0].astype(np.float32)
        self.view5 = data['X'][4][0].astype(np.float32)
    ...
    def __getitem__(self, idx):
        return [torch.from_numpy(self.view1[idx]), torch.from_numpy(self.view2[idx]), torch.from_numpy(self.view3[idx]), torch.from_numpy(
            self.view4[idx]), torch.from_numpy(self.view5[idx])], torch.from_numpy(self.labels[idx]), torch.from_numpy(np.array(idx)).long()
...
def load_data(dataset):
    if dataset == "Hdigit":
        ...
        dataset = YouTubeFace('data/YoutubeFace_sel_fea.mat')
